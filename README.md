...
**No LLM judge.** Deterministic failure detection. Runs in <50ms. Optional LLM layer for semantically ambiguous claims (opt-in, see [L3 configuration](#)).
...
```json
{
  "servers": {
    "stozer": {
      ...
      "env": {
        "STOZER_API_KEY": "stozer_live_your_key"
      }
    }
  }
}
...
**No LLM judge.** Deterministic core; optional LLM layer for unresolvable claims (opt-in). Supports 11 human languages.
